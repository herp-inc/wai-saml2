...
-- | Create a default 'AuthnRequest'
issueAuthnRequest :: T.Text -> IO AuthnRequest
issueAuthnRequest authnRequestIssuer = do
    ...
    pure AuthnRequest
        ...
        , authnRequestNameIDFormat = "urn:oasis:names:tc:SAML:2.0:nameid-format:transient"
        , ..
        }

-- | Generate a base64-encoded AuthnRequest for SP initiated SSO.
...
renderAuthnRequest AuthnRequest{..} = Base64.encode
    ...
    where
        timestamp = T.pack $ formatTime defaultTimeLocale iso8601Format authnRequestTimestamp
        root = Element
            (saml2pName "AuthnRequest")
            (Map.fromList [ ("xmlns:samlp", "urn:oasis:names:tc:SAML:2.0:protocol")
        ...
        issuer = Element (saml2Name "Issuer") mempty [NodeContent authnRequestIssuer]
        nameIdPolicy = Element
            (saml2pName "NameIDPolicy")
            (Map.fromList [ ("allowCreate", if authnRequestAllowCreate then "true" else "false")
