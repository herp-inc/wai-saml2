...
validateSAMLResponse cfg responseXmlDoc samlResponse now = do
    ...
    case responseSignature samlResponse of
        ...
        Nothing -> validateSAMLAssertionSignature responseXmlDoc samlResponse now cfg
...
validateSAMLAssertionSignature :: XML.Document -> Response -> UTCTime -> SAML2Config -> ExceptT SAML2Error IO Assertion
validateSAMLAssertionSignature responseXmlDoc samlResponse now cfg = do
